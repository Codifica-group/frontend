...
const AguardandoOrcamento = ({ onCardClick, currentPage, refreshTrigger, onTotalPagesChange }) => {
    ...
    const result = SolicitacoesBase({ 
        solicitacoes, 
        activeTab: "Aguardando orçamento", 
        onCardClick, 
        currentPage 
    });
    ...
    useEffect(() => {
        if (onTotalPagesChange) {
            onTotalPagesChange(result.totalPages);
        }
    }, [result.totalPages, onTotalPagesChange]);

    if (loading) {
        return <div className="loading-message">Carregando solicitações...</div>;
    }
...
export default AguardandoOrcamento;